...
const svgs = files.map(f => ({ name: f.replace('.svg', ''), content: fs.readFileSync(path.join(SVG_DIR, f), 'utf8').trim() }));
...
// Skip first 20 (already filled), start from index 20
const remaining = assigned.slice(20).map((svg, j) => ({ rectId: rectIds[20 + j], name: svg.name, svg: svg.content }));

const BATCH_SIZE = 6;
// Remove old batch files
const dir = 'D:\\WORK\\RIO hanoi\\Diben2025\\Icons_SVG';
fs.readdirSync(dir).filter(f => /^batch_\d+\.js$/.test(f)).forEach(f => fs.unlinkSync(path.join(dir, f)));

for (let i = 0; i < remaining.length; i += BATCH_SIZE) {
  const batch = remaining.slice(i, i + BATCH_SIZE);
  const bi = Math.floor(i / BATCH_SIZE) + 1;
  const code = `const data = ${JSON.stringify(batch.map(b => ({ rectId: b.rectId, name: b.name, svg: b.svg })))};
...
for (const item of data) {
  ...
  fs.writeFileSync(path.join(dir, `batch_${String(bi).padStart(2,'0')}.js`), code);
  console.log(`Batch ${bi}: ${batch.length} icons (~${Math.round(code.length/1024)}KB), rects ${20+i+1}-${20+i+batch.length}`);
}
console.log('Done');
