...
const assigned = rectIds.map(() => files[Math.floor(rand() * files.length)]);

const map = {};
for (let i = 56; i < rectIds.length; i++) { map[rectIds[i]] = assigned[i]; }
fs.writeFileSync('D:\\WORK\\RIO hanoi\\Diben2025\\Icons_SVG\\map_remaining.json', JSON.stringify(map));
console.log('entries:', Object.keys(map).length, '| rects 57 -', rectIds.length);
